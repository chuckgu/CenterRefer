...
def random_affine(img, mask, targets, centers, degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-2, 2),
    ...
    if centers is not None:
        ...
        centerw[0]=centers[0]
        centerw[1]=centers[1]
        centerw[2]=centers[0]+2
        centerw[3]=centers[1]+2
    ...
    if type(targets)==type([1]):
        ...
        targets = wrap_points(targets, M, height, a)
        centerw = wrap_points(centerw,M, height, a)
        return imw, maskw, targets,centerw[:2], M
    else:
        return imw
