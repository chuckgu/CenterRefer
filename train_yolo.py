...
def main():
    ...
    model = grounding_model(corpus=ifcorpus, light=args.light, emb_size=args.emb_size, coordmap=True, \
                            bert_model=args.bert_model, dataset=args.dataset, seg=args.seg, att=args.att)
    # model=model.cuda()
    ...
    if args.pretrain:
        if os.path.isfile(args.pretrain):
            ...
            pretrained_dict = {k: v for k, v in pretrained_dict.items()}  # if k in model_dict and 'refine' not in k}
...
def train_epoch(train_loader, model, optimizer, epoch, size_average):
    ...
    for batch_idx, (imgs, word_id, word_mask, bbox, mask, center) in enumerate(tbar):
        ...
        tbar.set_description(
            ...
            'EntLoss {ent_loss.avg:.4f} ' \
            'Accu_grid {acc_c.avg:.4f} ' \
                .format(seg_loss=losses, ent_loss=ent_losses, acc_c=acc_center))
...
def validate_epoch(val_loader, model, size_average, mode='val'):
    ...
    for batch_idx, (imgs, word_id, word_mask, bbox_gt, mask, center) in enumerate(tbar):
        ...
        with torch.no_grad():
            ## Note LSTM does not use word_mask
            pred_anchor, intmd_fea, flang, cam_out, cam_rv, attn_list = model(image, word_id, word_mask)
        # for ii in range(len(pred_anchor)):
...
def test_epoch(val_loader, model, size_average, mode='test'):
    ...
    for batch_idx, (imgs, word_id, word_mask, bbox_gt, ratio, dw, dh, im_id, mask, center, phrase, mask_origin,
        ...
        if point_vis:
            ...
            def vis_cam(cams, n, h, w, color, k, gj, gi, img, pred_conf_list, crf=True, entropy=True):
                # height = 256
                pred_ch = gj * k + gi
                ...
                if entropy:
                    ...
                    cam = F.interpolate(cam, (256, 256), mode='bilinear', align_corners=True)
                    ...
                    if crf:
                        ...
                        # cam = cam - np.min(cam)
                        # cam = cam / (np.max(cam)+1e-8)
                        cam = crf_inference(cam, 256, 256, crf_img)
                ...
                pred_conf = torch.softmax(pred_conf_list[n], dim=1)
                conf = pred_conf[:, pred_ch].item()
            ...
            # cam1, c1, conf1 = vis_cam(cam, 0, h, w, (123.7, 116.3, 103.5), 8, pred_gj[0], pred_gi[0], dst_back,
            #                           pred_conf_list, args.crf)
            # cam2, c2, conf2 = vis_cam(cam, 1, h, w, (123.7, 116.3, 140.5), 16, pred_gj[1], pred_gi[1], dst_back,
            #                           pred_conf_list, args.crf)
            # cam3, c3, conf3 = vis_cam(cam, 2, h, w, (123.7, 116.3, 103.5), 32, pred_gj[2], pred_gi[2], dst_back,
            #                           pred_conf_list, args.crf)
