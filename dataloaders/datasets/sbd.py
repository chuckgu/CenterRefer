...
class SBDSegmentation(BaseDataset):
    ...
    def __init__(
        ...
        for splt in self.split:
            lines = (self._dataset_dir / "{splt}.txt").read_text().splitlines()

            for line in lines:
                _image = self._image_dir / "{line}.jpg"
                _categ = self._cat_dir / "{line}.mat"
        ...
        # Display stats
        print("(sbd) Number of images: {len(self.images):d}")
    ...
    def __str__(self):
        return "SBDSegmentation(split={self.split})"
