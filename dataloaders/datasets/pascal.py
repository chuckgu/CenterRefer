...
class VOCSegmentation(BaseDataset):
    ...
    def __init__(
        ...
        lines = (_splits_dir / "{self.split}.txt").read_text().splitlines()

        for ii, line in enumerate(lines):
            _image = self._image_dir / "{line}.jpg"
            _cat = self._cat_dir / "{line}.png"
        ...
        # Display stats
        print("(pascal) Number of images in {split}: {len(self.images):d}")
    ...
    def __str__(self):
        return "VOC2012(split={self.split})"
